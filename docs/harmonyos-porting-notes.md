...
- 文本组件菜单中有 `autoFill` 和 `passwordVault`，但这是“被填充方 / 使用方”侧触发系统默认密码保险箱的能力。
- `@ohos.app.ability.autoFillManager` 目前公开正文里主要提供 `requestAutoSave`，用于由当前表单应用主动请求保存表单数据。
- `module.json5` 的 `extensionAbilities.type` 列表里确实出现 `autoFill/password` 和 `autoFill/smart`，但这只是 manifest 可识别类型列表，不等于普通三方应用已经获得 provider 开放能力。
- OpenHarmony 侧存在 `AutoFillExtensionAbility` 相关文档和示例，但相关文档带有系统接口、Full SDK、`system_basic` 权限、固定 `com.ohos.passwordbox` 等约束信号，不能直接外推到 HarmonyOS NEXT 普通 AppGallery 三方应用。
- ArkWeb 侧的 `requestPasswordAutoFill` 也是请求系统密码保险箱向当前焦点输入框填充，不代表 Bitwarden 类应用可以作为第三方 provider 被其他应用调起。

因此当前结论修正为：

- 截至当前复核，尚未确认 HarmonyOS NEXT 对普通三方密码管理器开放了等价 Android `AutofillService` 的公开 provider 接口。
- 现在能稳定确认的是“普通应用作为被填充方适配系统密码保险箱”的能力，而不是“普通三方密码管理器作为系统级密码 provider”的能力。
- 文档中不应再写“自动填充 provider 形态已确认可作为首选接入路径”。
- 自动填充仍是首发对齐目标，但它属于最高风险外部依赖；如果系统不开放 provider，首发必须改为受限等效方案并明确能力差异。

主要风险重新定义为“公开 provider 是否开放”，而不是“如何接入一个已确认开放的 provider”：

- 普通 AppGallery 三方应用能否声明 `extensionAbilities.type = "autoFill/password"` 并成功安装。
- 普通应用能否在公开 SDK 下编译 provider 生命周期接口。
- 普通应用是否需要特殊签名、ACL、Full SDK、系统权限、厂商合作或白名单。
- 系统设置里是否允许用户把普通三方密码管理器设置为默认密码保险箱 / 自动填充提供方。
- AppGallery 上架是否允许相关 extension、权限和能力声明。

所以，对“三方应用有自动填充能力”的现象，需要更谨慎地理解：

- 它可能是系统密码保险箱的使用方适配，也可能是厂商授权、特殊签名 / ACL、白名单或非公开接口。
- 即使某个同类应用在商店展示了自动填充能力，也不能直接证明公开 SDK 已经向所有普通三方密码管理器开放 provider。
- 在没有官方开放说明、真机 provider spike 和上架准入确认之前，不能把它写成“普通三方密码管理器一定可完整接入”。
...
- 当前没有证据证明 HarmonyOS NEXT 已向普通三方密码管理器开放等价 Android `AutofillService` 的 provider 接口。
- 因此首发架构应该保留 provider 抽象，但实现计划必须同时准备受限等效方案。

因此文档层面的正确表述应该是：

- 需求已确认：首发必须覆盖自动填充能力。
- 技术事实已更新：公开稳定能力主要是表单 / 使用方侧自动填充和请求保存。
- 技术事实待验证：普通 AppGallery 三方密码管理器是否存在公开 provider 入口。
...
- 首发范围必须包含自动填充目标，但不能承诺 Android 等价 provider 已可实现。
- 实现方案必须允许在“官方开放公开 provider 接入”“授权 / 特权 provider 接入”和“受限等效系统集成方案”之间切换。

进入架构设计前，自动填充需要做一个最小可行 spike：

1. 使用普通 HarmonyOS NEXT 应用包名声明 `extensionAbilities.type = "autoFill/password"`，确认是否能安装和被系统识别。
2. 在公开 SDK 下尝试编译 provider 侧生命周期接口；如果只能在 Full SDK 或系统接口下编译，应记录为“不对普通三方开放”。
3. 在真机系统设置中确认是否能把该应用设为密码保险箱 / 自动填充提供方。
4. 在其他 App 的 `TextInput`、ArkWeb、浏览器场景分别验证是否能触发填充 / 保存回调。
...
1. 完整 provider 接入，仅在后续确认鸿蒙允许普通三方密码管理器作为 provider 时采用。
...
1. HarmonyOS NEXT 当前能确认的是表单侧自动填充 / 密码保险箱使用方能力；尚未确认普通 AppGallery 三方密码管理器是否存在公开、可上架的 provider 接口。
...
- 当前证据只支持架构预留 provider 能力模块和受限等效方案，不足以承诺与 Android 的公开三方 provider 模型等价。
...
- 对自动填充、Passkey、凭据获取这类首发必做能力，架构上必须同时容纳“未来官方开放 provider 接入”“授权 / 特权 provider 接入”和“受限等效接入”三种实现路径。
- 不能因为平台能力尚未证实，就在架构上先把这些能力降出首发范围。
- 自动填充侧继续抽象 `AutofillProviderCapability`，但 `AutoFillExtensionAbility` / `autoFill/password` 只能作为待验证候选路径，不再写成已确认首选落点。
...
- HarmonyOS NEXT 当前可确认的是表单侧自动填充、密码保险箱触发和 `requestAutoSave` 这类使用方能力。
- 普通 AppGallery 三方密码管理器是否能作为自动填充 provider，被其他应用调起，当前仍未确认，并作为最高风险门槛保留。
...
- 鸿蒙自动填充 provider 的公开开放性仍未确认，风险重新收口为“普通三方应用是否存在公开 SDK、公开权限、普通签名和 AppGallery 可上架的 provider 路径”。
...
- OpenHarmony 自动填充 provider 相关资料（仅作为候选路径和风险参考，不能直接等同 HarmonyOS NEXT 普通三方开放能力）：
