...
1. Bitwarden internal SDK 的 Harmony 接入策略：
   ...
   - client 生命周期已确认：沿用 `globalClient + per-user cached client + single-use client`，并由 `SdkClientManager` 统一管理。
   - 宿主侧仓储桥接已确认：Android / SDK 职责对齐，Harmony 侧通过 adapter 封装。
   - 仍需进一步确认 Native 目标架构与运行时兼容性、认证与 Vault 能力覆盖、Passkey / FIDO2 桥接、SSO / Cookie 配置桥接、版本策略、测试策略。
2. 二期产品路线：
