...
- 自动填充首发目标继续保持。
- 架构设计优先按 provider 接入准备，不再把自动填充当作“可能完全没有入口”的能力。
...
1. HarmonyOS NEXT 已存在自动填充 provider 形态，但必须验证普通 AppGallery 三方密码管理器能否以自有包名、公开 SDK 和可上架权限完整接入。
...
- 鸿蒙自动填充 provider 已有明确形态，但普通三方应用的自有包名、公开 SDK、权限级别、签名、ACL 和 AppGallery 上架边界仍待验证。
- `hmbitwarden` 已在鸿蒙商店被观察到具备自动填充能力，这是同类应用正向证据；后续需要复核它的包名、实现路径、权限 / 签名 / ACL 条件和自动填充场景覆盖。
...
- 鸿蒙商店同类应用观察：
  - `hmbitwarden` 已被用户在鸿蒙商店观察到具备自动填充能力；公开 Web 入口暂未复核到详情，后续需要补充包名、版本、截图或商店详情页信息。
