...
1. 交付形态：
   ...
   - 首发阶段优先保证可调试、可定位 ABI / 加密 / Passkey / SSO / Vault 行为。
...
1. Bitwarden internal SDK 的 Harmony 接入策略：
   - 总体方向已确认：官方 SDK 直连优先。
   - 交付形态已确认：源码接入优先，稳定后再产物化。
   - 仍需进一步确认 ABI、client 生命周期、宿主侧仓储桥接、认证与 Vault 能力覆盖、Passkey / FIDO2 桥接、SSO / Cookie 配置桥接、版本策略、测试策略。
2. 二期产品路线：
