...
- 二期也不规划独立 `Authenticator` 或 `authenticatorbridge`。
...
1. 先确认最终目标是否需要独立 `Authenticator`。
2. 如果目标不明确，第一阶段建议优先做主密码管理器，并只为 TOTP 域保留轻量扩展边界。
...
- 二期也不规划独立 `Authenticator` 或 `authenticatorbridge`。
- 架构上只预留轻量扩展边界，不提前实现独立应用、独立锁态或 bridge 协议。
...
- `@ohos.app.ability.autoFillManager` 提供 `requestAutoSave`，用于由使用方应用主动请求保存表单数据。
- `module.json5` 的 `extensionAbilities.type` 明确包含 `autoFill/password` 和 `autoFill/smart`。
- OpenHarmony 的 `AutoFillExtensionAbility` 文档明确把自动填充拆成“提供方”和“使用方”，并提供 `onFillRequest`、`onSaveRequest` 等 provider 生命周期。
- ArkWeb 侧也有 `requestPasswordAutoFill`，可请求密码保险箱中的用户名或密码自动填充到当前焦点输入框。

因此当前结论需要修正为：

- HarmonyOS / OpenHarmony 侧确实存在自动填充 provider 形态，不能再简单写成“没有 provider 入口证据”。
- 这套形态在职责上更接近 Android `AutofillService`，至少覆盖账号密码的填充、保存、生成密码和选择 UI。
- 但它还不能直接等同于 Android 面向普通三方密码管理器开放的 `AutofillService` / `CredentialProviderService`。
- 结合 `hmbitwarden` 的鸿蒙商店观察，自动填充的风险判断应从“平台入口是否存在”调整为“普通三方密码管理器如何拿到稳定、可上架、可维护的 provider 接入”。

主要风险不再是“有没有 provider 模型”，而是下面这些更具体的准入问题：

- `AutoFillExtensionAbility` 在 API 参考中标注为系统接口。
- OpenHarmony 自动填充示例需要替换 Full SDK 才能编译。
- 示例说明涉及权限至少为 `system_basic` 级别。
- 示例还写明 `bundleName` 只能是 `com.ohos.passwordbox`，不可修改。
- 当前还没有验证普通 AppGallery 三方应用是否能用自己的包名注册 `autoFill/password` provider。

所以，对你看到“三方应用有自动填充能力”的现象，需要这样理解：

- 这是一个很有价值的正向信号，说明 HarmonyOS 生态里确实存在非系统主应用参与自动填充的可能性。
- 但它可能来自系统密码保险箱接入、厂商合作授权、特殊签名 / ACL、白名单或特定上架资质。
- 在没有真机 spike、上架准入确认和公开权限路径之前，不能把它直接写成“普通三方密码管理器一定可完整接入”。

#### 7.6.1 已观察到的市场证据：`hmbitwarden`

当前还有一个重要的市场侧观察：

- 用户已经在鸿蒙商店看到 `hmbitwarden` 实现了自动填充能力。
- 这说明“密码管理器类应用在鸿蒙商店实现自动填充”不是纯理论推断，已经有同类应用信号。
- 这会显著提高我们对首发自动填充可行性的判断权重。
- 公开 Web 复核时也能看到社区讨论提到 `HmBitwarden` 是“原版 Bitwarden 移植”，并带有鸿蒙商店截图；但该资料不是官方商店详情页，也没有直接说明自动填充实现细节，只能作为“商店内确有同类应用”的辅助证据。

但这条证据目前还需要继续拆解：

- 需要确认 `hmbitwarden` 的包名、版本号、上架主体和应用类别。
- 需要确认它是通过 `AutoFillExtensionAbility` / `autoFill/password` 实现，还是通过系统密码保险箱、授权能力、特定 ACL / 签名、辅助入口或其他私有接入方式实现。
- 需要确认它的自动填充是否支持其他三方 App、ArkWeb / 浏览器、保存新密码、生成密码、锁屏后解锁再填充等完整场景。
- 需要确认它是否支持 Passkey / password credential provider，还是仅支持账号密码自动填充。

因此，这条证据应该改变我们的工作优先级，但不能取消 spike：

- 自动填充首发目标继续保持。
- 架构设计默认按 provider 接入准备，不再把自动填充当作“可能完全没有入口”的能力。
- spike 的重点从“有没有 provider 能力”进一步收窄为“普通上架三方应用如何拿到同等能力”。
...
- 当前已经找到 HarmonyOS / OpenHarmony 的 provider 形态，因此首发架构应该优先按 provider 接入方向设计。
- 但这并不代表 HarmonyOS NEXT 已经被证实允许普通三方密码管理器以自有包名完整接入系统 provider。

因此文档层面的正确表述应该是：

- 需求已确认：首发必须覆盖自动填充能力。
- 技术事实已更新：系统存在自动填充 provider 形态。
- 技术事实待验证：普通 AppGallery 三方密码管理器是否能以自有包名、公开 SDK 和可上架权限完整接入。
...
- 首发范围必须包含自动填充，首选实现路径是 `AutoFillExtensionAbility` / `autoFill/password` provider 接入。
- 实现方案仍要允许在“公开三方 provider 接入”“授权 / 特权 provider 接入”和“等效系统集成方案”之间切换。

进入架构设计前，自动填充需要做一个最小可行 spike：

1. 使用普通 HarmonyOS NEXT 应用包名声明 `extensionAbilities.type = "autoFill/password"`。
2. 在公开 SDK 下尝试编译 `AutoFillExtensionAbility`、`FillRequest`、`SaveRequest` 等接口。
3. 在真机上安装后确认是否能被系统识别为密码保险箱 / 自动填充提供方。
4. 在其他 App 的 `TextInput`、ArkWeb、浏览器场景分别触发 `onFillRequest` / `onSaveRequest`。
5. 验证是否需要特殊签名、ACL、Full SDK、`system_basic` 权限或华为侧合作准入。
6. 验证 AppGallery 上架时是否允许声明相关 extension 和权限。

如果鸿蒙最终不存在完全对等的 provider 入口，可能的等效实现层级为：

1. 完整 provider 接入，如果后续验证鸿蒙允许普通三方密码管理器作为 provider。
2. 授权 / 特权 provider 接入，如果需要厂商合作、ACL 或特定签名。
3. 通过密码保险箱或系统支持入口完成部分集成。
4. 应用内搜索并手动复制 / 粘贴。
5. 从分享 / 拉起 / 中转页完成半自动填充。
...
- `FIDO2_PASSKEY_PLATFORM_AUTHENTICATOR`、`FIDO2_USER_VERIFYING_PLATFORM_AUTHENTICATOR` 等客户端能力枚举
- `FIDO2_PLATFORM` / `FIDO2_CROSS_PLATFORM` 认证器类型
- `USB / NFC / BLE / HYBRID / INTERNAL` 等认证器传输方式
- `ohos.permission.ACCESS_FIDO2_ONLINEAUTH` 权限要求

通行密钥错误码文档还明确提到“密码保险箱”存储限制：
...
- 鸿蒙具备平台认证器和密码保险箱侧的 Passkey 存储 / 使用能力。
- Bitwarden 鸿蒙版作为“登录 Bitwarden 自己账号的客户端”，接入设备内置 Passkey 登录的技术路径相对明确。

还不能直接推出：

- 鸿蒙一定存在与 Android Credential Manager 等价的第三方凭据提供框架。
- 鸿蒙一定支持第三方密码管理器对其他应用统一提供 Passkey。
- `AutoFillExtensionAbility` 的账号密码 provider 能力一定覆盖 Passkey provider 能力。
...
- 已确认的是产品目标。
- 已确认的是 HarmonyOS NEXT / Online Authentication Kit 具备 FIDO2 / 通行密钥客户端能力。
- 尚未确认的是 HarmonyOS NEXT 是否提供与 Android Credential Manager / Credential Provider 等价的第三方 password / Passkey 凭据提供框架。
...
第 1、2 层是首发必做且当前证据更充分；第 3 层是最高风险点，不能默认可行。
...
- 凭据提供方能力要单独抽象成 `CredentialProviderCapability`，不要和 `PlatformPasskeyCapability` 合并。
...
1. 设备内置 Passkey 登录 Bitwarden 账号的注册、断言、origin、权限和错误码映射。
2. SDK 的 `Fido2CredentialStore` / `Fido2UserInterface` 与 Harmony FIDO2 返回结构之间的桥接。
3. 系统是否允许 Bitwarden 作为第三方 password credential provider 被其他 App 调起。
4. 系统是否允许 Bitwarden 作为第三方 Passkey provider 为其他 App / Web 产生断言。
5. 如果 password provider 通过 `AutoFillExtensionAbility` 完成，Passkey provider 是否有同一入口、独立入口或根本不开放。
...
当前已经确认二期也不规划独立 `Authenticator` 或 `authenticatorbridge`。

因此一期架构不实现双 App、bridge 协议或独立锁态，但建议保留轻量扩展边界：

- TOTP 域不要和具体页面强绑定。
- 锁态、同步状态、导入导出能力保持可被独立 feature 复用。
- 外部入口、深链、扫码结果回流统一经过 root navigation / special circumstance 处理。
- 如果未来重新评估独立产品，也尽量不推翻一期的 Vault / TOTP 数据边界。
...
独立 `Authenticator` 和 `authenticatorbridge` 已确认不进入二期规划，因此当前不再维护二期 Authenticator 页面清单。

但主应用内的 TOTP 新增入口、二维码扫描、手动录入、验证码展示和复制体验仍然属于一期 Password Manager 页面范围。
...
1. HarmonyOS NEXT 已存在自动填充 provider 形态，且已有鸿蒙商店同类应用信号；但必须验证普通 AppGallery 三方密码管理器能否以自有包名、公开 SDK 和可上架权限完整接入。
2. HarmonyOS NEXT 已存在 FIDO2 / 通行密钥客户端能力，但必须验证第三方应用能否对其他应用统一提供 Passkey / password credential。
3. Bitwarden internal SDK 是否存在 Harmony 可接入路径。
4. 独立 `Authenticator` 与 bridge 路线已确认不纳入二期规划，但一期架构仍保留轻量扩展边界。

其中第 1、2 项尤其特殊：

- 它们已经是首发目标的一部分。
- 现在需要确认的不是“要不要做”，而是“普通三方上架应用是否能完整接入，以及如果需要特权 / 合作 / 白名单，要走哪种等效方案”。
- 当前证据已经足够支持架构预留 provider 能力模块，但还不足以承诺与 Android 的公开三方 provider 模型完全等价。
...
- 对自动填充、Passkey、凭据获取这类首发必做能力，架构上必须同时容纳“普通三方 provider 接入”“授权 / 特权 provider 接入”和“受限等效接入”三种实现路径。
- 不能因为平台能力尚未证实，就在架构上先把这些能力降出首发范围。
- 自动填充侧优先抽象 `AutofillProviderCapability`，首选落点是 `AutoFillExtensionAbility` / `autoFill/password`。
- 设备内置 Passkey 登录抽象为 `PlatformPasskeyCapability`，第三方凭据提供单独抽象为 `CredentialProviderCapability`。
...
- HarmonyOS / OpenHarmony 侧已确认存在自动填充 provider 形态，首选验证 `AutoFillExtensionAbility` / `autoFill/password` 接入。
- 普通 AppGallery 三方密码管理器是否能完整接入自动填充 provider，仍作为高风险门槛保留。
- 一期登录侧需要支持 `TOTP` 两步验证码和设备内置 Passkey（platform authenticator）登录。
- HarmonyOS / Online Authentication Kit 侧已确认存在 FIDO2 / 通行密钥客户端能力，设备内置 Passkey 登录优先按 `PlatformPasskeyCapability` 接入。
- 第三方 Passkey / password credential provider 能力仍需与自动填充分开验证。
- 外部安全密钥、`NFC`、`YubiKey` 这类外部 Passkey / WebAuthn 登录方式不纳入一期。
...
- 独立 `Authenticator` 和 `authenticatorbridge` 二期也不做，但保留 TOTP 域、锁态、同步和外部入口的轻量扩展边界。
...
后续不再把独立 `Authenticator` 或 `authenticatorbridge` 作为二期规划项；一期只保留轻量扩展边界，不实现独立应用、独立锁态或 bridge 协议。

### 12.3 当前最需要持续盯住的高风险门槛

- 鸿蒙自动填充 provider 已有明确形态，风险已经从“是否存在入口”收敛为“普通三方应用的自有包名、公开 SDK、权限级别、签名、ACL 和 AppGallery 上架边界能否满足”。
- `hmbitwarden` 已在鸿蒙商店被观察到具备自动填充能力，这是同类应用正向证据；公开 Web 资料也能辅助证明商店中存在 `HmBitwarden` 相关应用，但后续仍需要复核它的包名、实现路径、权限 / 签名 / ACL 条件和自动填充场景覆盖。
- 鸿蒙设备内置 Passkey / FIDO2 登录能力证据较充分，但第三方 Passkey / password credential provider 的真实边界仍待验证。
...
- HarmonyOS 密码保险箱 / 自动填充相关文档入口：
  ...
  - https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkweb-access-password-safe
- OpenHarmony 自动填充 provider 相关资料：
  - https://gitee.com/openharmony/docs/blob/f9a137e9b7fbe0410bcb7c66447bba25c901293c/zh-cn/application-dev/application-models/autofillextensionablility-guide.md
  - https://gitee.com/openharmony/docs/blob/f9a137e9b7fbe0410bcb7c66447bba25c901293c/zh-cn/application-dev/reference/apis-ability-kit/js-apis-app-ability-autoFillExtensionAbility-sys.md
  - https://gitee.com/openharmony/applications_app_samples/tree/master/code/SystemFeature/AutoFill/AutoFill
- 鸿蒙商店同类应用观察：
  - `hmbitwarden` 已被用户在鸿蒙商店观察到具备自动填充能力；公开 Web 入口未复核到官方商店详情，但已找到社区讨论提到 `HmBitwarden` 与鸿蒙商店截图，后续仍需要补充包名、版本、截图或商店详情页信息。
  - https://hu60.net/q.php/bbs.topic.107332.html
- HarmonyOS / OpenHarmony 通行密钥与 FIDO2 相关资料：
  - https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/onlineauthentication-passkey-arkts
  - https://developer.huawei.com/consumer/cn/doc/harmonyos-references/onlineauthentication_capi_header_fido2
