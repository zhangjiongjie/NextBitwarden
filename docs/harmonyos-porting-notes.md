...
- 仍然需要逐项验证 SDK 交付形态、宿主桥接接口、Passkey / SSO 配置桥接、ABI 与运行时约束。
...
1. 交付形态：
   - 我们到底拿什么接 Harmony。
   - 是直接消费现成产物，还是要自己编译 Rust / native library，再通过 C ABI / FFI / NAPI 桥到 ArkTS。
2. ABI 与运行时：
   - 是否支持 Harmony 首发目标设备需要的 ABI。
   - native library 的加载、初始化、生命周期和异常恢复方式是什么。
3. Client 生命周期：
   - Android 当前是 `globalClient + per-user cached client + single-use client`。
   - Harmony 是否也要保留同样的多账号 client 管理模型，以及 client close / destroy 的时机如何定义。
4. 宿主侧仓储桥接：
   ...
   - 这些接口在 Harmony 侧分别由哪些本地存储、账号状态和配置源来承接。
5. 认证与密码学能力覆盖面：
   - 是否完整覆盖登录注册、密码哈希、KDF、password strength、policy filter、PIN、trusted device、key connector、组织加密初始化等链路。
   - 不能只验证“能解密 Vault”，还要验证认证域是否够用。
6. Vault 核心能力覆盖面：
   - 是否完整覆盖 cipher / folder / send / attachment 的加解密、导入导出、密码历史、TOTP、更新密码、更新 KDF、组织内移动等能力。
   - 这些能力决定一期主应用是否能真正闭环。
7. Passkey / FIDO2 桥接：
   - Android 当前还给 SDK 提供了 `Fido2CredentialStore` 和 `Fido2UserInterface` 一类宿主实现。
   - Harmony 侧是否能把本地 Passkey / WebAuthn / 凭据选择流程等价映射给 SDK。
8. SSO / Cookie / Server Communication Config：
   ...
   - Harmony 侧浏览器拉起、Cookie 持久化、回调闭环是否能满足这部分接入要求。
9. 版本策略与可维护性：
   - 后续是跟 Android 一样持续跟进 SDK 版本，还是先锁定一个 Harmony 可用版本。
   - 每次 SDK 升级是否会引入新的 native 依赖、桥接接口或行为变更。
10. 测试与可替换性：
   - Harmony 侧是否有办法做 SDK mock、桥接层测试、跨语言集成测试。
   - 如果某一块能力暂时接不上，是否允许局部重写，同时保持后续可替换回官方 SDK。
...
4. 推送是走 Push Kit、轮询，还是双模式并存。
5. 一期之后是否还要规划独立 `Authenticator` 与 bridge 路线，以及现在的架构是否需要提前预留扩展边界。
...
1. Bitwarden internal SDK 的 Harmony 接入策略：
   - 总体方向已确认：官方 SDK 直连优先。
   - 仍需进一步确认交付形态、ABI、client 生命周期、宿主侧仓储桥接、认证与 Vault 能力覆盖、Passkey / FIDO2 桥接、SSO / Cookie 配置桥接、版本策略、测试策略。
2. 推送、实时同步、登录审批的首发策略：
   - 是否允许无推送降级。
   - 如果允许，轮询和时延要求是什么。
3. 二期产品路线：
