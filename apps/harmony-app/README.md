...
- `features`：Auth、Vault、Autofill、Passkey、Settings 的一期功能契约、首批状态模型、Auth / Vault mock repository 和系统集成 readiness repository。
- 首批 UI 状态流：登录、解锁、保险库首页、两步验证码、系统集成和设置概览。
- `ohosTest`：首批 Hypium 测试入口，当前覆盖 App 级 reducer、Auth / Vault 状态机、Auth / Vault mock repository 和系统集成 readiness repository。
